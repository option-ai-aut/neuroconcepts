        ...
        <div className="space-y-2">
          {ALERTS.filter(a => !a.acked).map((alert, i) => (
            <div key={i} className={`flex items-center gap-3 px-4 py-2.5 rounded-lg border ${
              alert.severity === 'warning' ? 'bg-amber-50 border-amber-200' : 'bg-gray-50 border-gray-200'
            }`}>
              <AlertTriangle className={`w-4 h-4 shrink-0 ${alert.severity === 'warning' ? 'text-amber-600' : 'text-gray-600'}`} />
            ...
            <div key={i} className="flex items-center gap-4 px-4 py-3 hover:bg-gray-50/50">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${dep.env === 'Production' ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-700'}`}>{dep.env}</span>
                </div>
