-- CreateEnum
DO $$ BEGIN
  CREATE TYPE "SalesStage" AS ENUM ('NEW_LEAD','DEMO_SCHEDULED','DEMO_DONE','PROPOSAL_SENT','NEGOTIATION','WON','LOST');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE "SalesActivityType" AS ENUM ('NOTE','EMAIL_SENT','CALL','DEMO','PROPOSAL','STAGE_CHANGE','MEETING','FILE_UPLOAD','TASK_DONE');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- Foreign Keys
ALTER TABLE "SalesActivity" ADD CONSTRAINT IF NOT EXISTS "SalesActivity_prospectId_fkey"
  FOREIGN KEY ("prospectId") REFERENCES "SalesProspect"("id") ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE "SalesTask" ADD CONSTRAINT IF NOT EXISTS "SalesTask_prospectId_fkey"
  FOREIGN KEY ("prospectId") REFERENCES "SalesProspect"("id") ON DELETE CASCADE ON UPDATE CASCADE;
